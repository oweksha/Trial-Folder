...
## variables with time. The function takes the input t (the current time), 
## y (a vector giving the current value of the state variables - that is, their
...
##-- Model 2: SIR model with births and deaths --##
##
## sir() -- Function for numerical analysis of model 1
sir.bd <- function(t, y, parms){
  ...
  # local environment created by the function
  with(c(as.list(y), parms),{
    births <- deaths*S + deaths*I + deaths*R
    dSdt <- -beta*S*I/N + births - deaths*S
    dIdt <- beta*S*I/N - gamma*I - deaths*I
    dRdt <- gamma*I - deaths*R
    list(c(dSdt, dIdt, dRdt))
  })
}
...
pop.SI <- c(S = 0.065*N0,  # Initially 6.5% of the population is susceptible
            I = 20.5, R = N0 - (0.065*N0)-20.5)       # ENTER THE NUMBER INITIALLY INFECTED (20.5)
...
values <- c(beta = 3.6,        # Transmission coefficient
            ...
            deaths = 1/(40*365))     # rate of deaths        
...
sir.bd(t = time, y = pop.SI, parms = values)
...
pop.next <- pop.SI + unlist(sir.bd(t = time, y = pop.SI, parms = values)) * delta.t
...
time.out <- seq(0, 1825, by = 0.1)     # INCLUDE THE APPROPRIATE VALUE TO GET A SEQUENCE
...
plot(ts.sir.bd$time,               # Time on the x axis
     ...
     main = "Measles in New York BD LE 40",    # Plot title
     xlim = c(0, 2000),           #
     ylim = c(0, 15000), 
     col = "red",
     type = "l",                # Use a line plot
     bty = "L",                 # Remove the box around the plot
     las = 1)                 
...
## For sure the epidemic is not coming back
## Question 2:
##
...
##
## Due to biths and deaths, the epidemic peak is lowered ie prevalnce decreases
## 
## Question 4:
##
